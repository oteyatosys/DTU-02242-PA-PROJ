...
class CallGraphPredictor(TestPredictor):
    ...
    def predict(self) -> Set[MethodSignature]:
        call_graph = build_call_graph(self.old_program)
        ...
        def walk_callgraph(start_node: MethodSignature, old_signature: MethodSignature) -> bool:
            ...
            if not self.new_program.contains_method(old_signature):
                print(f"Method {old_signature} has been removed")
                test_predictions.add(start_node)
                new_methods.add(old_signature)
                return False
            
            new_method = self.new_program.method(old_signature)
            old_method = self.old_program.method(old_signature)
        ...
        for file, method in self.new_program.all_test_methods():
